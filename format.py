...
class Format:
    ...
    def formatLine(self, curLine: str):
        ...
        if len(operands) != 0:
            lastOperand = (
                ","
                + " " * self.operandPreSpacing
                + operands.pop().ljust(self.operandSize)
            )
            if len(operands) != 0:
                for op in operands:
                    returnString += (
                        "," + " " * self.operandPreSpacing + op.ljust(self.operandSize)
                    )
    ...
    def startToDelimeter(
        self, string: str, delims: set
    ):  # returns tuple(str,str), where tuple[0] is the substring before delimeter, and tuple[1] is the remaining chars
        substring = ""
        for curChar in string:
